...
//      echo '<pre>';  print_r($order_goods);exit;
        
    
            $evaluate_goods_array = array();
            foreach ($order_goods as $value){
               
                if($value['goods_id'] == $temp[$value['goods_id']]['goods_id']){
                 
                    //如果未评分，默认为5分
                    $evaluate_score = intval($temp[$value['goods_id']]['score']);
                    if($evaluate_score <= 0 || $evaluate_score > 5) {
                         $evaluate_score = 5;
                    }
                    
                    //默认评语
                    $evaluate_comment = $temp[$value['goods_id']]['comment'];
