...
class member_buyControl extends mobileMemberControl {
    ...
    public function buy_step2Op() {
        ...
        //处理代金券
        $voucher = htmlspecialchars_decode(stripslashes($_POST['voucher']));
        if(is_null(@json_decode($voucher,TRUE))){
            $param['voucher'] = $_POST['voucher'];
        }else{
            $param['voucher'] = json_decode($voucher,TRUE);
        }
        
        //新增手机端选项
        $param['store_id']=$_POST['store_id'];
        $param['store_pstype']=$_POST['store_pstype'];
		$param['shipping_fees']=$_POST['shipping_fees'];
